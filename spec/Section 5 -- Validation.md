...
* If {directiveName} is "defer" or "stream":
  * The parent type of {directive} must not be {mutationType} or {subscriptionType}.
...
For example, the following document will not pass validation because `@defer` has
been used on a root mutation field:
...
For example, the following document will only pass validation if `field` is defined
